---
title: golang基础-包与依赖管理
lastmod: 2025-05-20T16:43:23+08:00
...
当引入的多个包中存在相同的包名或者想自行为某个引入的包设置一个新包名时，都需要通过`importname`指定一个在当前文件中使用的新包名
...
![包初始化函数执行顺序示意图](https://seranor-1251900471.cos.ap-chengdu.myqcloud.com/typora/package01.png)
...
国内加速地址`https://goproxy.cn`和`https://goproxy.io`
...
在 `holiday` 文件夹下使用下面命令初始化项目

```go
go mod init holiday
```

这个命令会在当前目录下创建一个 `go.mod` 的文件，内容如下
...
mkdir overtime
cd overtime
go mod init overtime
```

overtime/overtime.go
...
holidy/go.mod 文件中正常引入`liwenzhou.com/overtime`包

然后使用 `replace`语句将这个依赖替换为使用相对路径表示的本地包。
...
在`holiday/main.go`下正常引入并使用`overtime`包
...
![go module生成的版本信息组成示意图](https://seranor-1251900471.cos.ap-chengdu.myqcloud.com/typora/module_version_info.png)
...
打包代码发布到 `github.com` 上

创建仓库  `hello`

```shell
git clone https://github.com/xxx/hello
cd hello
```
...
func SayHi() {
	fmt.Println("你好，很高兴认识你。")
}
```

然后将该项目的代码 push 到仓库的远端分支，这样就对外发布了一个Go包。

其他的开发者可以通过`github.com/xxx/hello`这个引入路径下载并使用这个包了。
...
![语义化版本号示意图](https://seranor-1251900471.cos.ap-chengdu.myqcloud.com/typora/version_number.png)
...
module github.com/xxx/hello/v2
...
```bash
go get github.com/xxx/hello/v2@v2.0.0
```
...
import (
	...
	"github.com/xxx/hello/v2" // 引入v2版本
)
...
如果某个发布的版本存在致命缺陷不再想让用户使用时，我们可以使用`retract`声明废弃的版本。

例如我们在`hello/go.mod`文件中按如下方式声明即可对外废弃`v0.1.2`版本。

```go
module github.com/xxx/hello
