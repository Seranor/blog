...
相较于之的那些具体类型（字符串、切片、结构体等）更注重“我是谁”，接口类型更注重“我能做什么”的问题。
...
我们可不可以把所有动物都当成一个“会叫的类型”来处理呢？当然可以！使用接口类型就可以实现这个目标
...
![接口值示例](https://seranor-1251900471.cos.ap-chengdu.myqcloud.com/typora/interface01.png)
...
![接口值示例](https://seranor-1251900471.cos.ap-chengdu.myqcloud.com/typora/interface02.png)
...
![接口值示例](https://seranor-1251900471.cos.ap-chengdu.myqcloud.com/typora/interface03.png)
...
![接口值示例](https://seranor-1251900471.cos.ap-chengdu.myqcloud.com/typora/interface04.png)
...
接口值可能赋值为任意类型的值，我们可以借助标准库`fmt`包的格式化打印获取到接口值的动态类型。
...
由于接口类型变量能够动态存储不同类型值的特点，会滥用接口类型（特别是空接口）来实现编码过程中的便捷。
